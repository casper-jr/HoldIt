...
- **ROE(자기자본이익률)** - **[구현 완료: processor.py에서 net_income + total_equity로 계산]**
  * ROE = (당기순이익 / 자기자본) × 100
- **FCF(잉여현금흐름) 및 FCF 수익률** - **[구현 완료: KR=DART 현금흐름표 / US=yfinance cashflow]**
  * FCF = 영업현금흐름 - 설비투자(CapEx)
  * FCF 수익률 = (FCF / 시가총액) × 100
- **PEG(주가수익성장비율)** - **[구현 완료: KR·US 모두]**
  * PEG = PER ÷ EPS 연간 성장률(%)
  * US: yfinance annual income_stmt에서 EPS CAGR(최대 3년) 계산, 실패 시 `earningsGrowth` fallback
  * KR: DART `fnlttSinglAcnt.json` 단일 응답의 `frmtrm_amount`(전년) / `lwfr_amount`(전전년) 활용. `lwfr` 양수 시 2년 CAGR, 없으면 1년 YoY
  * 유효 조건: PER > 0 AND 성장률 > 0. 그 외(적자·역성장·데이터 없음) → NULL → 0점 처리
- **부채비율(재무 건전성)** - **[구현 완료: KR=DART 재무상태표 / US=yfinance quarterly_balance_sheet]**
  * 부채비율 = (총부채 / 자기자본) × 100
...
- **자사주 소각 여부** - **[구현 완료 : 매입+소각 모두 확인하는 로직 → 소각만 확인하도록 변경]**
...
3. 가공된 정보들을 이용하여 scoring_guidelines에 따른 점수 계산을 수행하여 결과를 날짜와 함께 저장
   - **멀티 scorer 아키텍처**: scorer 버전을 갈아 끼울 수 있도록 팩토리 패턴으로 구현. `scorer.py`의 `get_scorer(version)` 팩토리 함수로 버전 선택.
     - `scorer_v1` — main 브랜치 레거시 기준 (PER/PBR/배당 중심, 최대 47점)
     - `scorer_v2` — Buffett & Lynch 기반 현행 기준 (ROE/FCF/PEG/부채비율 추가, 최대 100점) **[기본값]**
     - 새 버전 추가 시: `scorer_vN.py` 생성(ScorerBase 상속) 후 `scorer.py`의 `_SCORERS` dict에 등록만 하면 됨
   - **scorer 버전 선택**: `python3 main.py score --all --scorer v1` 형태로 버전 지정. 미지정 시 v2 기본.
   - **버전별 독립 저장**: `scoring_results.scorer_version` 컬럼으로 같은 테이블에 v1/v2 레코드 공존. 기존 데이터에 영향 없음.
4. 이후에 점수 계산 결과를 조회할 수 있도록 하고, 필요시 각 종목의 세부지표들의 값과 지표별 점수 추이를 조회할 수 있도록 함
   ...
   - **scorer 버전 필터링**: `view --scorer v1`, `view --scorer v2`로 버전별 리더보드 조회 가능. 미지정 시 v2 기본. 헤더에 `[scorer v1/v2]` 표시.
   - **조회 필터링**: 정성평가 만점을 획득하더라도 70점(B등급) 미만인 종목은 리더보드(`view`)에서 자동으로 제외됨
   - **중복 방지**: `score`를 여러 날짜에 실행해도 `view`/`export`에서는 종목당 버전별 가장 최근 평가일의 결과 1건만 표시
...
- `total_liabilities` (NUMERIC): 총부채 (KR: DART API 재무상태표 / US: yfinance quarterly_balance_sheet `Total Liabilities Net Minority Interest`) - 부채비율 계산용
- `operating_cash_flow` (NUMERIC): 영업현금흐름 (KR: DART 현금흐름표 / US: yfinance cashflow `Operating Cash Flow`) - FCF 계산용
- `capital_expenditure` (NUMERIC): 설비투자, CapEx (KR: DART 현금흐름표 / US: yfinance cashflow `Capital Expenditure`) - FCF 계산용
- `eps_growth_rate` (NUMERIC): EPS 연간 성장률 (%) - PEG 계산용. US: yfinance annual EPS CAGR(최대 3년) 또는 `earningsGrowth` fallback. KR: NULL 유지 (P3 장기 과제)
...
- `roe` (NUMERIC): 자기자본이익률 (%) = (당기순이익 / 자기자본) × 100
- `fcf` (NUMERIC): 잉여현금흐름 = 영업현금흐름 - CapEx
- `fcf_yield` (NUMERIC): FCF 수익률 (%) = (FCF / 시가총액) × 100
- `peg_ratio` (NUMERIC): PEG = PER ÷ EPS 연간 성장률(%). KR·US 모두 실데이터 계산. 유효 조건 미충족 시 NULL
- `debt_ratio` (NUMERIC): 부채비율 (%) = (총부채 / 자기자본) × 100
...
- `scorer_version` (VARCHAR, DEFAULT 'v2'): 사용된 scorer 버전 식별자 (`v1`, `v2`, ...) **[신규]**
...
>
> **`scorer_version` 설계 의도**: v1/v2 레코드가 같은 테이블에 공존. `view`/`export`/`score` 명령어에서 `--scorer` 플래그로 버전 선택.
