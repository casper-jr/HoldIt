...
- 배당 인상 기록(지급 기록) - **[구현 완료: KR=DART alotMatter.json (최대 12년치) / US=yfinance 연도별 배당금 분석]**
  * *KR 참고: yfinance는 한국 주식 배당 이력을 3~5년치만 제공하는 경우가 많아 DART `alotMatter.json` (배당에관한사항)으로 대체. 각 API 호출은 당기/전기/전전기(thstrm/frmtrm/lwfr) 3개 연도를 반환하며, 4번 호출(기준연도, -3년, -6년, -9년)로 최대 12년치 이력 확보. 만점 기준(10년 이상 연속 인상) 충족 가능.*
...
5. **데이터 내보내기**: 수집 시점에서의 결과를 엑셀(CSV) 형식으로 Export하여, 사용자가 직접 정성적 평가 항목의 점수를 기입하고 최종 점수를 확인할 수 있도록 지원
   ...
   - **항목 구성**: 정량 항목별로 측정값과 점수를 분리 출력 (예: PER 값 + PER점수(/20)), 정성 평가 6개 항목은 빈칸, 최종_예상점수 열에 엑셀 수식(정량합계 + 정성항목 SUM) 자동 삽입, 맨 마지막 열(메모)은 정성 평가 근거 등 자유 형식 노트용 빈칸
...
- `dividend_increase_years` (INTEGER): 배당 연속 인상 연수 (KR: DART `alotMatter.json` 주당 현금배당금 기준 최대 12년치 / US: yfinance 연도별 배당금 분석). KR은 직전 완료 연도(last_year)부터 역방향으로 연속 증가 연수를 계산하며, 전년 대비 감소 또는 데이터 공백이 있으면 연속이 끊긴 것으로 간주.
