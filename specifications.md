...
- EPS(가장 최근 분기 또는 작년 값 사용, 당기순이익/발행주식 으로 계산) - **[구현 완료: KR=DART API 사업보고서 / US=yfinance income_stmt]**
  * *참고: 장기투자 관점에서 분기별 실적 변동에 의한 왜곡을 막기 위해 항상 가장 최근의 '사업보고서(1년 치 최종 실적)'를 기준으로 계산하도록 수정됨.*
- PER(현재값/EPS 로 계산) - **[구현 완료]**
- PBR(가장 최근 분기 값) - **[구현 완료: KR=DART API 자본총계 / US=yfinance balance_sheet]**
- 배당수익률(한 주당 연간 배당금 / 현재 주가 x 100) - **[구현 완료: yfinance 작년도 연간 총 배당금]**
  * *참고: 배당금 역시 EPS와 마찬가지로 장기투자 관점에서의 일관성을 위해 최근 1년(TTM) 기준이 아닌 '작년도 연간 총 배당금'을 확정값으로 사용합니다.*
...
1. 국내/해외 상장된 주식 정보들을 불러와서 DB에 불러온 날짜 정보와 함께 저장
   - **한국 (KR)**: 시가총액 기준 상위 300~500개 종목 권장 (배당 없는 종목 자동 제외 → 실제 100~200개 내외로 압축)
   - **미국 (US)**: yfinance screener를 통해 NYSE/NASDAQ 전체를 시가총액 순으로 수집 (배당 없는 종목 자동 제외)
...
4. 이후에 점수 계산 결과를 조회할 수 있도록 하고, 필요시 각 종목의 세부지표들의 값과 지표별 점수 추이를 조회할 수 있도록 함
   - **시장 필터링**: `view kr`, `view us`, `view`(전체)로 시장별 리더보드 조회 가능. 개수 지정도 가능 (예: `view us 100`)
...
- `net_income` (NUMERIC): 당기순이익 (KR: DART API / US: yfinance income_stmt)
- `total_equity` (NUMERIC): 자본총계 (KR: DART API / US: yfinance balance_sheet) - PBR 계산용
...
- `treasury_shares` (NUMERIC): 자사주 보유 수 (KR: DART API 주식총수현황 / US: yfinance balance_sheet)
